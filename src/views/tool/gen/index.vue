        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="数据源" prop="dataName">
              <el-select v-model="queryParams.dataName" filterable clearable placeholder="请选择/输入数据源名称" disabled>
...
const handleGenTable = async (row?: TableVO) => {
  ...
  if (row?.genType === '1') {
    ...
    proxy?.$download.zip('/tool/gen/batchGenCode?tableIdStr=' + tbIds, row.tableName + '.zip');
  }
};
/** 同步数据库操作 */
